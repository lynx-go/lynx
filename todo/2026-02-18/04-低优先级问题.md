# 低优先级问题

这些是代码完善性改进，不影响核心功能，但解决后可提高代码质量。

---

## 问题 1: RequestLogger mutex 被注释

### 位置
- `server/http/requestlog.go:17,44-47`

### 问题描述

`sync.Mutex` 相关代码被注释掉，看起来是不完整的重构：

```go
// server/http/requestlog.go:14-22
type RequestLogger struct {
    onErr func(error)

    //mu     sync.Mutex  // ❌ 被注释
    logger *slog.Logger
    //w   io.Writer     // ❌ 被注释
    //buf bytes.Buffer   // ❌ 被注释
    //enc *json.Encoder  // ❌ 被注释
}

// server/http/requestlog.go:43-47
func (l *RequestLogger) log(ent *requestlog.Entry) error {
    //defer l.mu.Unlock()  // ❌ 被注释
    //l.mu.Lock()          // ❌ 被注释
    // ...
}
```

### 影响

1. **代码不完整** - 注释的代码让人困惑
2. **潜在的并发问题** - 如果将来需要并发写入
3. **文档意义不明** - 不清楚是否需要这些字段

### 修复建议

```go
// 方案 1: 完全移除注释代码 (推荐)
type RequestLogger struct {
    onErr  func(error)
    logger *slog.Logger
}

func (l *RequestLogger) log(ent *requestlog.Entry) error {
    // slog.Logger 本身是并发安全的，不需要额外锁
    var r struct { ... }
    // ... 日志逻辑
    l.logger.Debug("requestlog", "request", r)
    return nil
}

// 方案 2: 如果需要恢复并发保护
type RequestLogger struct {
    mu     sync.Mutex
    onErr  func(error)
    logger *slog.Logger
}

func (l *RequestLogger) log(ent *requestlog.Entry) error {
    l.mu.Lock()
    defer l.mu.Unlock()
    // ... 日志逻辑
}
```

---

## 问题 2: Listener 未显式管理

### 位置
- `server/grpc/server.go:112-124`

### 问题描述

`net.Listen` 创建的 listener 没有显式管理，依赖 `GracefulStop` 间接关闭：

```go
// server/grpc/server.go:109-125
func (s *Server) Start(ctx context.Context) error {
    log.InfoContext(ctx, "starting gRPC server, listening on "+s.o.Addr)

    lis, err := net.Listen("tcp", s.o.Addr)  // ❌ listener 未保存
    if err != nil {
        return err
    }

    s.health.SetServingStatus("grpc", grpc_health_v1.HealthCheckResponse_SERVING)
    reflection.Register(s.server)
    s.running.Store(true)
    return s.server.Serve(lis)
}
```

### 影响

1. **资源管理不明确** - listener 生命周期不透明
2. **难以调试** - 无法查看监听状态
3. **潜在泄漏** - 如果 Serve 异常退出

### 修复建议

```go
type Server struct {
    server   *grpc.Server
    listener net.Listener  // 添加 listener 字段
    logger   *slog.Logger
    o        Options
    health   *health.Server
    running  atomic.Bool
}

func (s *Server) Start(ctx context.Context) error {
    log.InfoContext(ctx, "starting gRPC server, listening on "+s.o.Addr)

    lis, err := net.Listen("tcp", s.o.Addr)
    if err != nil {
        return fmt.Errorf("failed to listen on %s: %w", s.o.Addr, err)
    }
    s.listener = lis  // 保存 listener

    s.health.SetServingStatus("grpc", grpc_health_v1.HealthCheckResponse_SERVING)
    reflection.Register(s.server)
    s.running.Store(true)
    return s.server.Serve(lis)
}

func (s *Server) Stop(ctx context.Context) {
    log.InfoContext(ctx, "stopping gRPC server")

    if s.health != nil {
        s.health.SetServingStatus("grpc", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
    }
    s.running.Store(false)

    // 先关闭 listener，停止接受新连接
    if s.listener != nil {
        s.listener.Close()
    }

    // 然后优雅关闭服务器
    if s.server != nil {
        // ... 带超时的 GracefulStop
    }
}
```

---

## 问题 3: 缺少单元测试

### 位置
- 整个代码库

### 问题描述

当前代码库没有任何单元测试文件 (`*_test.go`)。

### 影响

1. **回归风险** - 重构可能引入 bug
2. **信心不足** - 无法验证修复正确性
3. **文档缺失** - 测试是最好的使用文档

### 修复建议

参见 [06-测试建议.md](./06-测试建议.md)。

---

## 问题 4: 错误变量未导出

### 位置
- 多个文件

### 问题描述

一些错误值作为局部变量或未导出变量，无法被调用者比较：

```go
// 当前实现没有定义可比较的错误类型
// 例如：ErrNotInitialized, ErrTimeout 等
```

### 修复建议

```go
// 定义可导出的错误变量
var (
    ErrNotInitialized = errors.New("component not initialized")
    ErrTimeout        = errors.New("operation timeout")
    ErrCancelled      = errors.New("operation cancelled")
)

// 或使用 errors.New 创建哨兵错误
// errors.go
package lynx

import "errors"

var (
    ErrConfigNotFound = errors.New("config not found")
    ErrComponentFailed = errors.New("component failed")
)
```

---

## 问题 5: 缺少文档注释

### 位置
- 多个文件

### 问题描述

部分公开 API 缺少 GoDoc 注释：

```go
// lynx.go - 部分函数有注释，部分没有

// ❌ 缺少注释
func IDFromContext(ctx context.Context) string {
    return ctx.Value(keyId).(string)
}

// ❌ 缺少注释
func (app *lynx) Hooks(hooks ...HookOption) error {
    // ...
}
```

### 修复建议

```go
// IDFromContext returns the instance ID from the context.
// If the ID is not set, it returns an empty string.
func IDFromContext(ctx context.Context) string {
    if v := ctx.Value(keyId); v != nil {
        if s, ok := v.(string); ok {
            return s
        }
    }
    return ""
}

// Hooks registers lifecycle hooks and components with the application.
// Multiple HookOption values can be passed to register multiple hooks
// or components in a single call.
//
// Example:
//
//  app.Hooks(
//      lynx.OnStart(startFunc),
//      lynx.OnStop(stopFunc),
//      lynx.Components(httpServer, grpcServer),
//  )
func (app *lynx) Hooks(hooks ...HookOption) error {
    // ...
}
```

---

## 问题 6: Options 结构体字段缺少验证

### 位置
- `options.go`, 多个组件的 Options

### 问题描述

Options 结构体没有验证逻辑，可能导致无效配置：

```go
// options.go - 缺少验证
type Options struct {
    Name         string
    ID           string
    Version      string
    CloseTimeout time.Duration
    // ...
}

func (o *Options) EnsureDefaults() {
    // 只设置默认值，没有验证
    if o.Name == "" {
        o.Name = "lynx"
    }
    // ...
}
```

### 修复建议

```go
type Options struct {
    Name         string        `validate:"required,min=1,max=63"`
    ID           string        `validate:"omitempty,hostname"`
    Version      string        `validate:"omitempty,semver"`
    CloseTimeout time.Duration `validate:"omitempty,min=1s,max=5m"`
}

func (o *Options) Validate() error {
    if o.Name == "" {
        return errors.New("name is required")
    }
    if len(o.Name) > 63 {
        return errors.New("name must be at most 63 characters")
    }
    if o.CloseTimeout < 0 {
        return errors.New("close timeout must be positive")
    }
    if o.CloseTimeout > 5*time.Minute {
        return errors.New("close timeout must be at most 5 minutes")
    }
    return nil
}

func (o *Options) EnsureDefaults() error {
    if o.Name == "" {
        o.Name = "lynx"
    }
    if o.ID == "" {
        hostname, _ := os.Hostname()
        o.ID = hostname
    }
    if o.CloseTimeout == 0 {
        o.CloseTimeout = 30 * time.Second
    }
    return o.Validate()
}
```

---

## 问题 7: 硬编码的魔法值

### 位置
- 多个文件

### 问题描述

代码中存在硬编码的魔法值：

```go
// lynx.go:140-145
func DefaultSetFlagsFunc(f *pflag.FlagSet) {
    f.StringP("config", "c", "", "config file path")
    f.String("config-type", "yaml", "config file type, default yaml")  // ❌ 硬编码 "yaml"
    // ...
}

// server/http/server.go:61
options := Options{
    Addr:    ":8080",  // ❌ 硬编码端口
    Timeout: time.Second * 60,  // ❌ 硬编码超时
}

// server/grpc/server.go:54
options := Options{
    Addr:    ":9090",  // ❌ 硬编码端口
    Timeout: time.Second * 60,  // ❌ 硬编码超时
}
```

### 修复建议

```go
// 定义常量
const (
    DefaultHTTPAddr    = ":8080"
    DefaultGRPCAddr    = ":9090"
    DefaultTimeout     = 60 * time.Second
    DefaultConfigType  = "yaml"
    DefaultCloseTimeout = 30 * time.Second
)

// 使用常量
func NewServer(handler http.Handler, opts ...Option) *Server {
    options := Options{
        Addr:    DefaultHTTPAddr,
        Timeout: DefaultTimeout,
        Logger:  slog.Default(),
    }
    // ...
}
```

---

## 问题汇总表

| 问题 | 严重程度 | 影响 | 修复难度 | 状态 |
|------|---------|------|---------|------|
| RequestLogger mutex 注释 | 🟢 低 | 代码不整洁 | 低 | ✅ 已修复 |
| Listener 未管理 | 🟢 低 | 资源管理不明确 | 低 | ✅ 已修复 |
| 缺少单元测试 | 🟢 低 | 回归风险 | 高 | ⏸️ 长期任务 |
| 错误变量未导出 | 🟢 低 | 无法比较错误 | 低 | ✅ 已修复 |
| 缺少文档注释 | 🟢 低 | 文档不完善 | 中 | ✅ 已修复 |
| Options 缺少验证 | 🟢 低 | 可能无效配置 | 低 | ✅ 已修复 |
| 硬编码魔法值 | 🟢 低 | 维护困难 | 低 | ✅ 已修复 |

---

## 修复优先级

这些是低优先级的改进，可以在有时间时逐步完善：

1. **快速修复** (1-2 小时) ✅ 已完成
   - 移除注释代码 ✅
   - 定义常量替换魔法值 ✅
   - 添加错误变量 ✅

2. **中等投入** (半天) ✅ 已完成
   - 添加文档注释 ✅
   - 添加 Options 验证 ✅
   - 管理 Listener 生命周期 ✅

3. **长期投入** (持续) ⏸️ 进行中
   - 编写单元测试 ⏸️
   - 完善测试覆盖率 ⏸️
