...
- `main.go:99-115`：自定义 flag 与 `WithBindConfigFunc`，启用 `LYNX_` 环境变量前缀并绑定 `LYNX_ADDR`。
- `otel.go:18 setupOTel`：初始化 stdout trace exporter 与 Prometheus meter provider，shutdown 挂到 `OnStop`。
- `main.go:76`：`/metrics` 暴露 Prometheus 指标（演示用，挂在主路由上）。
- `main.go:78-86`：`http.NewServer` 注入 TracerProvider/MeterProvider/Propagator 与 `latencyMiddleware`。
