...
- `main.go:36`：注册 `telemetry.New()`（`contrib/telemetry`）——以**服务**形式托管 OTel（Init 创建 provider 并设为全局，Stop 自动 flush；默认 trace 为 noop，需要本地调试时加 `telemetry.WithStdoutTrace()`）。
...
- `main.go`：`/metrics` 经 `http.WithEndpoint("/metrics", telemetry.PrometheusHandler())` 独立挂载——抓取流量不经过业务中间件与 otel instrumentation；自定义指标与运行时指标一起导出。
- `main.go`：`http.NewServer` 注册 HTTP 服务；otel provider 走全局（OTel 服务设置），无需逐项传入。
