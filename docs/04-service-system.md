...
go get github.com/lynx-go/lynx/contrib/cluster
...
> **v1.16 消费模型（破坏性）**：订阅单元是事件（逻辑 topic）——同一事件的多个 handler 共享一条 transport 订阅并进程内并行扇出；消费组 / 消费者成员数是后端配置（kafka `consumer.*`）；Bus 层旋钮为 `bus.topics.<t>.max_in_flight`（在途上限，默认 1，串行且保序）与 `bus.handler_timeout`（handler 单次尝试超时，默认不限制）。从旧模型（`WithGroup` / `WithInstances` / `WithTopicGroup`）迁移见 `CHANGELOG.md` Unreleased 与 [design-eventbus-consumption.md](design-eventbus-consumption.md) §6。
