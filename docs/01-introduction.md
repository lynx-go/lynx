# 1. 项目简介

## 1.1 什么是 Lynx

Lynx 是一个轻量级的 Go 微服务框架，旨在提供开箱即用的应用生命周期管理和服务化开发体验。它吸收了多个优秀开源项目的精华，提供了一个简洁、灵活且强大的开发框架。

## 1.2 设计理念

### 简洁至上
Lynx 遵循"简单即美"的设计哲学，提供最小化的核心 API，开发者可以快速上手并专注于业务逻辑的实现。

### 服务化架构
通过统一的 `Service` 接口，Lynx 将所有可管理的功能单元抽象为服务，实现了高度的模块化和可扩展性。

### 优雅的生命周期管理
内置完善的启动/停止流程，支持优雅关闭（Graceful Shutdown），确保服务在启停过程中不丢失数据和请求。

### 生产就绪
集成了健康检查、日志、配置管理等生产环境必需的功能，让开发者能够快速构建生产级别的微服务。

## 1.3 核心特性

### 应用生命周期管理
- 简洁的启动/停止流程
- 支持优雅关闭
- 钩子机制（Hooks）支持自定义生命周期逻辑

### 服务系统
- 基于 `Service` 接口的插件化架构
- 服务自动注册和管理
- 按注册顺序初始化，依赖通过初始化顺序表达

### 服务器支持
- **HTTP 服务器**：内置 HTTP 服务器，支持健康检查、请求日志
- **gRPC 服务器**：内置 gRPC 服务器，支持拦截器、健康检查、反射服务
- **HTTP/gRPC 客户端**：内置客户端组件，支持 otel 插装、trace/日志属性
  传播（request_id/user_id）、超时与重试默认值

### 健康检查
- 内置 `lynx.Checker` 健康检查机制（`CheckHealth() error`）
- 实现 `Checker` 的服务注册时自动收集，经 `app.HealthCheckers()` 暴露
- 便于监控和服务发现集成

### 配置管理
- 基于 Viper 的灵活配置系统
- 统一 `Config`/`ConfigSource` 接口访问，底层配置库可替换
- 支持多来源配置（命令行、环境变量、配置文件）
- 支持多种配置格式（JSON、YAML、TOML 等）

### 事件驱动
- 一等 EventBus：`Bus` / `Topic[T]` / `Event[T]`（默认内存，开箱即用）
- 跨进程：`contrib/watermill` Bus + `contrib/watermill-kafka` Transport
- 类型化 `Topic.Publish` / `Subscribe`；生命周期事件 `lynx.*` 强制进程内内存 Transport

### 定时任务
- 基于 Cron 的调度器
- 支持任务注册和管理
- 灵活的任务执行控制

### 日志集成
- 支持 `log/slog` 标准库
- 集成 `zap` 高性能日志库
- 结构化日志支持

### CLI 模式
- 支持命令行工具开发
- 单命令执行（`app.Command`）
- 命令行参数绑定

### 依赖注入
- 支持 Wire 依赖注入
- 自动生成依赖代码
- 编译时依赖检查

## 1.4 适用场景

Lynx 适用于以下场景：

### 微服务开发
快速构建和部署微服务，内置的服务器服务和健康检查机制让微服务开发变得简单。

### CLI 工具开发
构建命令行工具和后台服务，统一的 CLI 接口简化了命令行应用的开发。

### 事件驱动应用
构建基于事件的异步处理系统，内置的 PubSub 和 Kafka 支持让消息处理变得简单。

### 定时任务系统
开发需要定时执行任务的应用，内置的调度器支持灵活的任务管理。

## 1.5 范围边界

Lynx 的价值在生命周期与服务抽象，保持核心精简，以下方向明确不在框架范围内：

- **数据层**：不提供数据库/Redis 客户端集成、ORM 或迁移工具——业务自行选型（gorm/sqlx/pgx 等），连接池等资源的关闭挂在 `OnPostStop` 清理钩子即可纳入框架生命周期管理。
- **配置中心**：不默认集成 Apollo/Nacos 等，按需以 contrib 提供；核心只抽象 `Config`/`ConfigSource` 接口。
- **脚手架工具**：代码生成与项目脚手架属于外部工具链，非框架组件。
- **动态插件**：扩展方式是编译期的 `Service`/`ServiceFactory` 与 contrib 模块，不做运行时插件加载。

## 1.6 技术栈

Lynx 构建于以下优秀的开源项目之上：

- **Go 1.25+**：编程语言
- **Viper**：配置管理
- **pflag**：命令行参数解析
- **Wire**：依赖注入
- **gRPC**：RPC 框架
- **Zap**：高性能日志库
- **Cron**：定时任务调度

## 1.7 项目结构

```
lynx/
├── boot/           # 应用引导和依赖注入
├── eventbus/       # EventBus（Bus / Topic / Event）
├── client/         # HTTP/gRPC 客户端
│   ├── grpc/       # gRPC 客户端
│   └── http/       # HTTP 客户端
├── contrib/        # 扩展服务
│   ├── watermill/       # Watermill 驱动的 Bus
│   ├── watermill-kafka/ # Kafka Transport
│   ├── registry/   # 服务注册发现
│   ├── consul/     # Consul 后端（注册发现 + cluster.Coordinator）
│   ├── cluster/    # 进程间协调
│   ├── cluster-redis/
│   ├── schedule/   # 定时任务
│   ├── telemetry/  # OpenTelemetry 生命周期托管
│   └── zap/        # Zap 日志集成
├── docs/           # 文档
├── server/         # 服务器实现
│   ├── http/       # HTTP 服务器
│   └── grpc/       # gRPC 服务器
└── _examples/      # 示例代码
```

## 1.8 快速体验

### 安装

```bash
go get github.com/lynx-go/lynx
```

### 最简单的 HTTP 服务

```go
package main

import (
    "context"
    gohttp "net/http"

    "github.com/lynx-go/lynx"
    "github.com/lynx-go/lynx/server/http"
)

func main() {
    router := gohttp.NewServeMux()
    router.HandleFunc("/", func(w gohttp.ResponseWriter, r *gohttp.Request) {
        w.Write([]byte("Hello, Lynx!"))
    })

    cli := lynx.NewRunner(func(app lynx.App) error {
        app.Register(http.NewServer(router, http.WithAddr(":8080")))
        return nil
    },
        lynx.WithName("my-app"),
        lynx.WithVersion("1.0.0"),
    )

    cli.Run()
}
```

运行：
```bash
go run main.go
```

访问 http://localhost:8080 即可看到结果。

## 1.9 下一步

- [第 2 章：快速开始](./02-quick-start.md) - 学习如何创建第一个 Lynx 应用
- [第 3 章：核心概念](./03-core-concepts.md) - 了解 Lynx 的核心设计理念
- [第 4 章：服务系统](./04-service-system.md) - 深入理解服务化架构
- [第 5 章：服务器](./05-servers.md) - 学习 HTTP 和 gRPC 服务器使用
- [第 6 章：客户端](./06-clients.md) - 学习 HTTP/gRPC 客户端与传播闭环
- [第 7 章：服务注册与发现](./07-registry.md) - 可选的注册发现 contrib（memory/DNS/Consul）与排水注销时序
- [示例代码](../_examples/) - 查看更多实际应用示例
