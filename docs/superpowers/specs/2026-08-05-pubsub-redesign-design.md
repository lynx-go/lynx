...
9. `ConsumerOptions` 新增 `AutoCommitEnabled *bool`（`auto_commit_enabled`，nil = 保持 sarama 默认 true）→ `Consumer.Offsets.AutoCommit.Enable`。语义依据 watermill-kafka v3 `subscriber.go:594-597`：Enable=false 时 watermill 在每条消息 Ack 时显式 `sess.Commit()`（此时 `commit_interval` 不生效）；true（默认）依赖 sarama 后台按 `Interval` 定时提交。指针类型避免零值歧义（bool 零值 false 无法表达"未设置"）。
