...
本仓库是多模块 Go workspace：根模块 `github.com/lynx-go/lynx` 与 4 个 contrib 子模块共存于同一仓库。发布时**必须为每个模块单独打 tag**，否则 Go 模块代理无法解析子模块版本。
...
1. 按 `Version` 逐个执行 `git tag -a` + `git push origin`，共 5 次（根 + 4 个 contrib）；
...
- [ ] **CI 全绿**：`.github/workflows/ci.yml` 中 6 个模块（根、`_examples`、4 个 contrib）的 vet、`go test -race` 与 golangci-lint 矩阵全部通过
- [ ] **本地回归**：6 个模块逐个执行 `go build ./... && go vet ./... && go test -race ./... && golangci-lint run`
