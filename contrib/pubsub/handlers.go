...
//
// 开发者通常无需手动实现本接口：使用 NewHandler（类型化）或
// NewRawHandler（原始字节）工厂构造。
...
// NewHandler 创建类型化事件处理器：消息 Payload 经 topic 的 Marshaler
// 反序列化到 T 后调用 handle。元数据（ID/Key/Headers）随 TypedMessage
// 信封传入，无需额外上下文查询。
func NewHandler[T any](topic, handlerName string, handle func(ctx context.Context, event *TypedMessage[T]) error, opts ...SubscribeOption) Handler {
...
// NewRawHandler 创建原始字节事件处理器：Handle 直接收到 *Message。
func NewRawHandler(topic, handlerName string, handle func(ctx context.Context, event *Message) error, opts ...SubscribeOption) Handler {
	return &rawHandler{
		topic: topic, handlerName: handlerName,
		handle: handle, opts: opts,
	}
...
// typedHandler 是 NewHandler[T] 的擦除实现：类型参数 T 仅存在于
...
// rawHandler 是 NewRawHandler 的实现：NewEvent 返回 nil 表示原始语义，
// Handle 收到 *Message。
